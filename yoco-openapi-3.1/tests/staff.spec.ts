...
test.describe('GET /v1/staff/{staff_id}', () => {
  ...
  test('[ST-05] @extended Given a pre-existing staff member ID, when fetching staff member by ID, then returns 200 with matching id', async ({ request }) => {
    const resourceId = process.env.TEST_STAFF_ID;
    test.skip(!resourceId, 'Skipped: set TEST_STAFF_ID to a real staff member ID to execute');

    const response = await request.get(`${API_BASE}/v1/staff/${resourceId}`, {
      headers: authHeaders(),
    });
    expect(response.status()).toBe(200);
    const body: unknown = await response.json();
