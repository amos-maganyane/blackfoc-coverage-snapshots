...
test.describe('GET /accounts/{accountId}/transaction-request-types', () => {
  ...
  test('[TT-05] @smoke Given a newly created account, when requesting transaction request types, then returns 200', async ({ request }) => {
    ...
    const response = await request.get(
      `${API_BASE}/accounts/${accountId}/transaction-request-types`,
      { headers: authHeaders() },
    );
